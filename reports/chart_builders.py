...
def _package_order(df: pd.DataFrame) -> list[str]:
    if "Account_3" not in df.columns or "MTH_Price" not in df.columns:
        return []

    package_totals = (
        df.groupby("Account_3", dropna=False)[["MTH_Price", "YTD_Price"]]
        .sum()
        .reset_index()
    )
    package_totals["has_value"] = package_totals[["MTH_Price", "YTD_Price"]].abs().max(axis=1)
...
def get_all_package_order(df: pd.DataFrame) -> list[str]:
    return _package_order(df)


def _package_order_for_charts(df: pd.DataFrame) -> list[str]:
    return _package_order(df)
...
def _build_waterfall(df: pd.DataFrame, level: str, perf_col: str, title: str) -> go.Figure:
    label_order = REPORT_ZONE_ORDER if level == "Zone" else _package_order_for_charts(df)
    fig = build_waterfall_figure(df, level, perf_col, title, total_label="ABI", label_order=label_order)
    return fig


def _build_zone_table(df: pd.DataFrame, perf_col: str, title: str) -> matplotlib.figure.Figure:
